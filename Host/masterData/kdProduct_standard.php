...
if($submit=='Save')
{
			...
			header("location:kdProduct_standard.php?no=9"); 
			}
			else
			{				
		...
		for($j=0;$j<$cnt;$j++){
			    ...
			    mysql_query("INSERT INTO kd_product(Kd_Category,UOM1,Product_code,Effective_date,product_type,Status_kd_product)VALUES('$Kd_Category','$UOM1','$Product_code','$Effective_date','Standard','Checked')");
				mysql_query("UPDATE product SET 
					  Status='Checked'
					  WHERE Product_code = '$Product_code'"); 
  				header("location:kdProduct_standard.php?no=1");
				}
				else
				{
				header("location:kdProduct_standard.php?no=51");exit;
				}
		}
			}
		}
			...
			header("location:kdProduct_standard.php?no=2");
			}
			}
//echo "select * from  kd_product where Kd_Category= '".$_GET['data']."'";
$list=mysql_query("select * from  kd_product where Kd_Category= '".$_GET['data']."'"); 
while($row = mysql_fetch_array($list)){ 
$Effective_date_db= $row['Effective_date'];
} 
$Effective_date_now=date('Y-m-d');
?>
<!------------------------------- Form -------------------------------------------------->
...
<table width="100%" align="center">
  <tr>
    ...
    <td width="121">
    <select name="Kd_Category" class="Kd_Category" id="Kd_Category"  autocomplete="off" style="width:100%;" value="" onChange="ajaxcategory_standard();">
	...
	<td width="165" class="align">Product Type</td>
		<td width="192">			
			...
			<option value="">--- Select ---</option>
			<option value="kdProduct_standard.php" selected>Standard</option>
...
<?php
		$qry="SELECT  * FROM `product` where product_type='Standard' order by id  asc";
...
<!-- 		<th>Edit</th>
 -->		<th>Mod</th>
		...
		<td>
		<input type="text" id="date_<?php echo $cc; ?>" class="Effective_date_update" value="<?php if($Effective_date_now!=$Effective_date_db){echo date("Y-m-d");}else{ echo $Effective_date_db;}?>" name="Effective_date" disabled>
		</td>
	   <td>
		<a id="link_<?php echo $cc; ?>" href="kdProduct_standard.php?Product_code=<?php echo $fetch['Product_code'];?>&hid_id=<?php echo $cc; ?>" class="test" onClick="inputcheck1(<?php echo $cc; ?>);">
...
<input type="button" name="clear" value="Clear" id="clear" class="buttons" onclick="return kdpdt_standard();" />&nbsp;&nbsp;&nbsp;&nbsp;
