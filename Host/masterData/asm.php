...
$sel_query=mysql_query($sel);
        ...
        $sql="INSERT INTO `asm_sp`(`KD_Code`,`KD_Name`,`DSR_Code`,`DSRName`,`Contact_Number`,`email_id`,`RSM`)
        values('$KD_Code','$KD_Name','$DSR_Code','$DSRName','$Contact_Number','$email_id','$RSM')";
		mysql_query( $sql);
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headingspr"  style="padding-right:10px;">ASM</div>
...
<table>
    ...
    <tr>
     ...
     <td>
        ...
        ?>
        <select name="RSM" id="rsm" onChange="return rsmcode();">
...
<div id="containerpr">
		...
		$var = @$_GET['DSRName'] ;
        $trimmed = trim($var);	
	    $qry="SELECT * FROM `asm_sp` where DSRName like '%".$trimmed."%' order by DSRName asc";
		}
		else
		{ 
	    $qry="SELECT *  FROM `Asm_sp` order by DSRName asc"; 
		}
