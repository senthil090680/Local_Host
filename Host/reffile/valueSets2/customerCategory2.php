...
$sql =("UPDATE category2 SET 
       ...
       WHERE id = $id");
mysql_query( $sql);
header("location:customercategory2.php?no=2");
}
else{
header("location:customercategory2.php?no=18");
}
}
}
...
$sel_query=mysql_query($sel);
        ...
        mysql_query( $sql);
        header("location:customercategory2.php?no=1");
		}
		else {
		header("location:customercategory2.php?no=18");
	}
...
<input type="reset" name="reset" id="reset"  class="buttons" value="Clear" id="clear" onclick="return cat2Clear();" />&nbsp;&nbsp;&nbsp;&nbsp;
...
<input type="text" name="category2" value="<?php $_GET['category2']; ?>" autocomplete='off' placeholder='Search By Cate1'/>
...
<div id="container">
		...
		if($cnt=='1'){
        header("location:customercategory2.php?no=33"); 
		  }
		}
		if($_GET['delID']!=''){
        ...
        header("location:customercategory2.php?no=3");
		}
		 }
		...
		$var = @$_GET['category2'] ;
        $trimmed = trim($var);		
	    $qry="SELECT * FROM `category2` where category2 like '%".$trimmed."%' order by id asc";
		}
		else
		{ 
		$qry="SELECT * FROM `category2` order by id asc";  
		}
		...
		<td align="right">
        <a href="customercategory2.php?id=<?php echo $fetch['id'];?>"><img src="../images/user_edit.png" alt="" title="" width="11" height="11"/></a>&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="customercategory2.php?id=<?php echo $fetch['id'];?>&delID=<?php echo $fetch['id'];?>"><img src="../images/trash.png" alt="" title="" width="11" height="11" /></a>
        </td>
        </tr>
		<?php $c++; $cc++; }		 
