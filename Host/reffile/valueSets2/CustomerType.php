...
$sql =("UPDATE customer_type SET 
       ...
       WHERE id = $id");
mysql_query( $sql);
header("location:CustomerType.php?no=2");
}
else{
header("location:CustomerType.php?no=18");
}
}
}
...
$sel_query=mysql_query($sel);
        ...
        mysql_query( $sql);
        header("location:CustomerType.php?no=1");
		}
		else {
		header("location:CustomerType.php?no=18");
	}
...
<div id="container">
		...
		if($cnt=='1'){
        header("location:CustomerType.php?no=35"); 
		  }
		}
		if($_GET['delID']!=''){
        ...
        header("location:CustomerType.php?no=3");
		}
		 }
        ...
        $trimmed = trim($var);		
	    $qry="SELECT * FROM `customer_type` where customer_type like '%".$trimmed."%' order by id asc";
		}
		else
		{ 
		$qry="SELECT * FROM `customer_type` order by id asc";  
		}
		...
		<td align="right">
        <a href="CustomerType.php?id=<?php echo $fetch['id'];?>"><img src="../images/user_edit.png" alt="" title="" width="11" height="11"/></a>&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="CustomerType.php?id=<?php echo $fetch['id'];?>&delID=<?php echo $fetch['id'];?>"><img src="../images/trash.png" alt="" title="" width="11" height="11" /></a>
        </td>
        </tr>
		<?php $c++; $cc++; }		 
