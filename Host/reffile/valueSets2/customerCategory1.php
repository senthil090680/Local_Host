...
<div id="container">
		...
		if($cnt=='1'){
        header("location:customerCategory1.php?no=32"); 
		  }
		}
		if($_GET['delID']!=''){
        ...
        header("location:customerCategory1.php?no=3");
		}
		 }
		...
		$var = @$_GET['category1'] ;
        $trimmed = trim($var);		
	    $qry="SELECT * FROM `category1` where category1 like '%".$trimmed."%' order by id asc";
		}
		else
		{ 
		$qry="SELECT * FROM `category1` order by id asc";  
		}
        ...
        <thead>
		<tr>
		<th class="rounded">Customer Category1<img src="../images/sort.png" width="13" height="13" /></th>
        ...
        <a href="customerCategory1.php?id=<?php echo $fetch['id'];?>&delID=<?php echo $fetch['id'];?>"><img src="../images/trash.png" alt="" title="" width="11" height="11" /></a>
        </td>
        </tr>
		<?php $c++; $cc++; }		 
