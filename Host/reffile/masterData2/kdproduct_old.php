...
if($_GET["Product_code"]!='')
{
	...
	if($row2=='0')
	{
			...
			$delete="delete from `kd_product` WHERE `Product_code` = ".$_GET['Product_code']; 
			$delete1=mysql_query($delete) or die(mysql_error()); 
			header('Location:kdProduct.php?no=3');
			}
			
	}
	else
	{
		header('Location:kdProduct.php?no=50');
	}
...
<!-- 		<th>Edit</th>
 -->		<th>Del</th>
	   ...
	   <td>
		<a href="kdProduct.php?Product_code=<?php echo $fetch['Product_code'];?>">
...
<a href="../include/menu.php" style="text-decoration:none"><input type="button" name="cancel" id="cancel"  class="buttons" value="Cancel"/></a></td>
     ...
     <input type="button" name="cancel" value="Cancel" class="buttons" onclick="window.location='kdProduct.php'"/>
        </form>
     </div>  
</div>
