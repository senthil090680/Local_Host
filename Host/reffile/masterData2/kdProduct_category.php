...
if($_GET["data"]!='')
{			
		$qry1="select * from kd_product where Kd_Category = '".$_GET['data']."' AND Status_kd_product='Checked'";
		...
		$upd_qry="UPDATE kd_product SET 
		Status_kd_product='UnChecked'
		WHERE Kd_Category = '".$_GET['data']."' AND Product_code='$Product_code'";
		$upd_qry1=mysql_query($upd_qry) or die(mysql_error()); 
		mysql_query("UPDATE product SET Status='UnChecked' WHERE Product_code = '$Product_code'"); 
		header('Location:kdProduct_category.php?no=2');
		}
		}
		else
		{
...
if($submit=='Save')
{
			...
			header("location:kdProduct_category.php?no=9"); 
			}
			else
			{				
		...
		for($j=0;$j<$cnt;$j++){
				$Product_code=$_POST['Product_code'][$j];
				$sel1="select * from kd_product where Product_code ='$Product_code' AND Kd_Category='$Kd_Category' AND Status_kd_product='Checked'"; 
				...
				mysql_query("UPDATE product SET 
  				...
  				header("location:kdProduct_category.php?no=1");
				}
				else
				{
				header("location:kdProduct_category.php?no=51");exit;
				}
		}
			}
		}
...
<table width="100%" align="center">
  <tr>
    <td width="157">KD Category*<input type="hidden" name="update_date" value="" autocomplete="off" id="hdnData" readonly></td>
    <td width="121">
    <select name="Kd_Category" class="Kd_Category" id="Kd_Category"  autocomplete="off" style="width:100%;" value="" onChange="ajaxcategory();">
	...
	<td width="165" class="align">Product Type</td>
		<td width="192">
		<select name="product_type" id="user"  autocomplete="off" style="width:100%;" value="" ONCHANGE="location = this.options[this.selectedIndex].value;">
		<option value="">--- Select ---</option>
		<option value="kdProduct_standard.php">Standard</option>
		<option value="kdProduct_POSM.php">POSM</option>
		<option value="kdProduct.php" selected>ALL</option>
		</select>
			</td>
  </tr>
</table>
...
<!-- 		<th>Edit</th>
 -->		<th>Mod</th>
		...
		<tr>
		<td>
		<input type="hidden" name="hid_cat" class="hid_cat" value="">
		<input type="checkbox" id="prd_code_<?php echo $cc; ?>" onClick="inputcheck(<?php echo $cc; ?>);" name="Product_code[]" value="<?php echo $fetch['Product_code'];?>" class="Product_code" <?php if($_GET['no']=='1'||$_GET['data']!=''){echo $fetch['Status'];}else{ echo 'Checked';}?>></td>
	   ...
	   <td>
		<a id="link_<?php echo $cc; ?>" href="kdProduct.php?Product_code=<?php echo $fetch['Product_code'];?>&hid_id=<?php echo $cc; ?>" class="test" onClick="inputcheck1(<?php echo $cc; ?>);">
...
<input type="button" name="clear" value="Clear" id="clear" class="buttons" onclick="return kdProduct_category();" />&nbsp;&nbsp;&nbsp;&nbsp;
