...
$sql=("UPDATE sales_representative SET 
		  ...
		  WHERE id = $id");
mysql_query( $sql);
header("location:SalesRep.php?no=2");
}
else{
...
if($Salesperson_id=='' || $salesperson_name=='' || $salesperson_cont_num=='' || $DSR_mapped=='')
{
header("location:SalesRep.php?no=9");exit;
}
	if (strpos($salesperson_email_id, "@")!== false)
		{
			// VALID
			if( !preg_match("/^[_a-z0-9-]+(.[_a-z0-9-]+)*@[a-z0-9-]+(.[a-z0-9-]+)*(.[a-z]{2,3})$/i", $userNameEmail) )
			{
					$Invalid = "Invalid email ID";
					header("location:SalesRep.php?no=11");exit;
			}
   		}
else{		
$sel="select * from sales_representative where Salesperson_id ='$Salesperson_id'";
$sel_query=mysql_query($sel);
        ...
        values('$Salesperson_id','$salesperson_name','$salesperson_email_id', '$salesperson_cont_num','$DSR_mapped','$active')";
        mysql_query( $sql);
        header("location:SalesRep.php?no=1");
		}
		else {
		header("location:SalesRep.php?no=18");
		}
    }
}
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headingspr">Sales Representative</div>
...
<div id="containerpr">
		...
		if($cnt=='1'){
        header("location:SalesRep.php?no=49"); 
		  }
		}
		if($_GET['delID']!=''){
        ...
        header("location:SalesRep.php?no=3");
		}
		 }
        ...
        $trimmed = trim($var);	
	    $qry="SELECT * FROM `sales_representative` where salesperson_name like '%".$trimmed."%' order by id asc";
		}
		else
		{ 
	    $qry="SELECT *  FROM `sales_representative` order by id asc"; 
		}
		...
		<td><?php echo $fetch['salesperson_cont_num'];?></td>
       	<td align="right" width="100"><a href="SalesRep.php?id=<?php echo $fetch['id'];?>"><img src="../images/user_edit.png" alt="" title="" width="11" height="11"/></a>&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="SalesRep.php?id=<?php echo $fetch['id'];?>"><img src="../images/trash.png" alt="" title="" width="11" height="11" /></a>
        </td>
        </tr>
		<?php $c++; $cc++; }		 
