...
<div id="mainarea">
<div class="mcf"></div>
<h3 align="center">DSR Master</h3>
<div>&nbsp;</div>
<div>&nbsp;</div>

		<form action="" method="get" name="master" id="master">
		<div id="searchReport">
		<label class="labelstyle">DSR Code / Name &nbsp;<input type="text" name="Name&Code" value="<?php echo $_GET['Name&Code']; ?>" /></label> &nbsp;
		...
		if($_GET['Name&Code']!='')
		{
		$where.="Salesrep_name LIKE '%".$_GET['Name&Code']."%' || DSR_id  LIKE '%".$_GET['Name&Code']."%' ";
	    $qry="SELECT *  FROM `dsr` where $where  order by id asc";
		}
		...
		<th>KD Code</th>
   		<th scope="col" class="rounded roundimage">SalesRepresentative Name</th>
		...
		<th scope="col" class="rounded">SalesPerson ID</th>
