...
<div>&nbsp;</div>
		...
		<div id="searchReport">
		<label class="labelstyle">DSR Code &nbsp;<input type="text" name="Name&Code" value="<?php echo $_GET['Name&Code']; ?>" /></label> &nbsp;
		...
		if($_GET['Name&Code']!='')
		{
		$where.="DSR_id LIKE '%".$_GET['Name&Code']."%'";
	    $qry="SELECT *  FROM `receipts` where $where  order by id asc";
		}
		...
		{ 
		$qry="SELECT *  FROM `receipts` order by id asc";
		}
		...
		<thead>
		<tr>
   		<th scope="col" class="rounded roundimage">DSR Code<br /><img src="../images/sort.png" width="13" height="13" /></th>
		<th>KD Code</th>
   		<th scope="col" class="rounded">Time</th>
		<th scope="col" class="rounded">GPS</th>
		<th scope="col" class="rounded">Receipt Serial Number</th>
        <th scope="col" class="rounded">Customer Code</th>
		...
		<td><?php echo $fetch['Receipt_Serial_Number'];?></td>
