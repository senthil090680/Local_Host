...
<div id="mainarea">
<div class="mcf"></div>
<h3 align="center">DSR Remarks</h3>
<div>&nbsp;</div>
<div>&nbsp;</div>
		<form action="" method="get" name="master" id="master">
		<div id="searchReport">
		<label class="labelstyle">DSR Code / Name &nbsp;<input type="text" name="Name&Code" value="<?php echo $_GET['Customer_Name']; ?>" /></label> &nbsp;
		...
		if($_GET['Name&Code']!='')
		{
		$where.="DSR_id LIKE '%".$_GET['Name&Code']."%' || Customer_Name LIKE '%".$_GET['Name&Code']."%' ";
	    $qry="SELECT *  FROM `dsr_comments` where $where  order by id asc";
		}
		...
		$qry="SELECT *  FROM `dsr_comments` order by id asc";
		}
